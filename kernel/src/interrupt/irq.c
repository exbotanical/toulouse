...
void
irq_exec_bottom_half (sigcontext_t *sc) {
  interrupt_bh_t *bh;
...
void
irq_handle_spurious_interrupt (int irq_num) {
...
void
irq_handler (int irq_num, sigcontext_t sc) {
  interrupt_t *irq;
  ...
  if (!irq) {
    irq_handle_spurious_interrupt(irq_num);
    goto done;
  }
