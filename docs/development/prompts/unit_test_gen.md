...
```
write unit tests for the following code. assume kmalloc and kmemset are basically malloc and memset and that the interrupt macros will be overidden so we dont have to worry about them in the test environment. model your tests like

static void abc_test(void) { // test code }
static void xyz_test(void) { // test code }

Use c and programming best practices and make sure tests exist for every edge case, failure mode, and happy case.
