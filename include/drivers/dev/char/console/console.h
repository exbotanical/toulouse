...
#define CONSOLE_LINES      video.rows
#define CONSOLE_SIZE       (video.columns * video.rows)
...
#define VCONSOLE_BUF_LINES (video.rows * MAX_SCROLLBACK_SCREENS)
#define VCONSOLE_BUF_SIZE  (video.columns * VCONSOLE_BUF_LINES)
 ...
 * TODO: ???
 */
extern short int *vconsole_screen[NUM_VCONSOLES + 1];
...
/**
 * Removes the CONSOLE_CLEARED flag, restores the video screen, and enables the cursor.
 */
void vconsole_unclear_screen(vconsole_t *vc);
